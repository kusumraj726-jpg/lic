                            ...
                            @forelse($clients as $client)
                                    ...
                                    "policies" => $client->renewals->map(fn($p) => [
                                        ...
                                        'expiry' => $p->expiry_date
                                                                    ...
                                                                    @php
                                                                        $policyLines = $client->renewals->map(fn($r) => "• " . $r->policy_number . " (" . $r->policy_type . ")")->implode("\n");
                                                                        $waMessage = "Hello " . $client->name . ",\n\n" .
                                                                                    "Your registration details are:\n" .
                                                                                    "ID: NX-100" . $client->id . "\n" .
                                                                                    "Email: " . ($client->email ?: 'N/A') . "\n" .
                                                                                    "Phone: " . ($client->phone ?: 'N/A') . "\n\n" .
                                                                                    "Registered Policies:\n" . ($policyLines ?: 'No policies registered') . "\n\n" .
                                                                                    "Thank you!";
                                                                        $waPhone = preg_replace('/[^0-9]/', '', $client->phone);
                                                                        // Add default country code if missing (assuming India +91 if 10 digits)
                                                                    ...
                                                                    <button @click="$dispatch('open-messaging-modal', { 
                                                                         title: 'Send Registration Info', 
                                                                         message: @js($waMessage), 
                                                                         phone: '{{ $waPhone }}', 
                                                                         name: '{{ addslashes($client->name) }}' 
                                                                     })" 
                                                                         class="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-all"
