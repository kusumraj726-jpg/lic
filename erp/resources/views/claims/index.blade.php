            ...
            <div class="flex items-center justify-between mb-10 px-2">
                ...
                <div class="flex items-center gap-4">
                    <form action="{{ route('claims.index') }}" method="GET" class="relative group">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search policy # or client..." class="!pl-16 pr-4 py-2.5 text-xs rounded-xl border-slate-100 dark:border-slate-700 focus:border-rose-500 focus:ring-rose-500 bg-white dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500 shadow-sm w-72 transition-all uppercase font-bold tracking-tight">
                    </form>
                    <a href="{{ route('claims.create') }}" class="inline-flex items-center gap-2 bg-rose-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-200 dark:shadow-none hover:bg-rose-500 transition-all">
                                ...
                                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/80 transition-all duration-300 group">
                                    ...
                                    <td class="px-8 py-6 text-right">
                                        <div class="flex items-center justify-end gap-3 text-xs font-black uppercase tracking-widest">
                                            ...
                                            @if($claim->client && $claim->client->phone)
                                                @php
                                                    $waPhone = preg_replace('/[^0-9]/', '', $claim->client->phone);
                                                    if (strlen($waPhone) === 10) { $waPhone = "91" . $waPhone; }
                                                    $waMessage = "Hello " . $claim->client->name . ",\n\n" .
                                                                "Regarding your claim for Policy #" . $claim->policy_number . " (" . $claim->policy_type . "):\n" .
                                                                "Claim Amount: ₹" . number_format($claim->claim_amount, 2) . "\n" .
                                                                "Status: " . strtoupper($claim->status) . "\n" .
                                                                "Incident Date: " . ($claim->incident_date ? $claim->incident_date->format('d M Y') : 'N/A') . "\n\n" .
                                                                "Thank you!";
                                                    $waUrl = "https://wa.me/" . $waPhone . "?text=" . urlencode($waMessage);
                                                @endphp
                                                <button @click="$dispatch('open-messaging-modal', { 
                                                    title: 'Send Claim Update', 
                                                    message: @js($waMessage), 
                                                    phone: '{{ $waPhone }}', 
                                                    name: '{{ addslashes($claim->client->name ?? '') }}' 
                                                })" 
                                                    class="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-all" title="Send via WhatsApp">
                    ...
                    <form :action="`/claims/${claim.id}`" method="POST" x-ref="editForm" @submit.prevent="submitForm">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label class="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-2 block">Claim Amount (₹)</label>
                                    <template x-if="mode === 'view'">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-2 block">Incident Date</label>
                                    <template x-if="mode === 'view'">
