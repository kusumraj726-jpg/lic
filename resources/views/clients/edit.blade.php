        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('clients.update', $client) }}" method="POST" enctype="multipart/form-data" class="space-y-8" 
                    x-data="{ 
                        policies: {{ json_encode($policies->map(fn($p) => [
                            'id' => $p->id,
                        ...
                        addPolicy() {
                            this.policies.push({ id: '', number: '', type: 'Life Insurance', premium: '', expiry: '', custom_type: '' });
                        },
                        removePolicy(index) {
                            ...
                            <div class="h-24 w-24 rounded-[2rem] bg-gradient-to-br from-indigo-500 to-purple-600 p-1 shadow-xl transition-transform group-hover:scale-105">
                                <div class="h-full w-full rounded-[1.75rem] bg-white dark:bg-slate-900 flex items-center justify-center overflow-hidden border-4 border-white dark:border-slate-800">
                                    <div id="photo-preview" class="h-full w-full flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 text-slate-300 dark:text-slate-500">
                                        @if($client->photo)
                                            <img src="{{ $client->photo_url }}" class="h-full w-full object-cover">
                                        @else
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <x-form-input label="Full Name" name="name" :value="old('name', $client->name)" required />
                        <x-form-input label="Email Address" name="email" :value="old('email', $client->email)" type="email" />
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <x-form-input label="Date of Birth" name="dob" :value="old('dob', $client->dob)" type="date" />
                        ...
                        <x-form-input label="Phone Number" name="phone" :value="old('phone', $client->phone)" placeholder="10-digit number" minlength="10" maxlength="10" pattern="[0-9]{10}" />
                    </div>

                    <div class="grid grid-cols-1 gap-8">
                        <div class="form-group flex-1">
                            <label for="address">Residential Address</label>
                            <textarea id="address" name="address" class="form-control dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500" rows="2">{{ old('address', $client->address) }}</textarea>
                    ...
                    <div class="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Manage and update all policies for this client.</p>
                            </div>
