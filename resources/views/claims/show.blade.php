    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3">
                <a href="{{ route('claims.edit', $claim->id) }}" class="px-6 py-2.5 rounded-2xl bg-indigo-600 text-white text-xs font-black uppercase tracking-widest hover:bg-indigo-700 transition-all">
                    Edit Record
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="premium-card bg-gradient-to-br from-indigo-600 to-indigo-900 border-none shadow-xl text-white">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            ...
                            <div class="px-8 py-4 rounded-[2rem] bg-white/10 backdrop-blur-md border border-white/20">
                                ...
                                <div class="text-xl font-black uppercase tracking-widest text-center @if($claim->status == 'approved') text-emerald-300 @elseif($claim->status == 'rejected') text-rose-300 @else text-amber-300 @endif">
                                    {{ $claim->status }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="premium-card bg-white border-none shadow-xl">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <p class="text-2xl font-black text-slate-900 tracking-tight dark:text-slate-100">₹ {{ number_format($claim->claim_amount, 2) }}</p>
                                </div>
                                <div>
                        ...
                        <div class="premium-card bg-white border-none shadow-xl">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <p class="text-lg font-black text-slate-900 dark:text-slate-100">{{ \Carbon\Carbon::parse($claim->incident_date)->format('d M, Y') }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Logged At</p>
                                    <p class="text-lg font-black text-slate-900 dark:text-slate-100">{{ $claim->created_at->format('d M, Y H:i') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="premium-card bg-white border-none shadow-xl">
                        ...
                        <div class="p-6 rounded-2xl bg-slate-50 border border-slate-100 text-slate-600 leading-relaxed italic dark:text-slate-300 dark:bg-slate-800/50">
                ...
                <div class="space-y-8">
                    ...
                    <div class="premium-card bg-white border-none shadow-xl">
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h4 class="text-lg font-black text-slate-900 uppercase tracking-tight dark:text-slate-100">{{ $claim->client->name }}</h4>
                                <p class="text-xs font-bold text-slate-400">{{ $claim->client->email }}</p>
                            </div>
                        </div>
                        <div class="pt-6 border-t border-slate-50">
                            <a href="{{ route('clients.show', $claim->client_id) }}" class="flex items-center justify-center w-full py-3 rounded-xl bg-slate-50 text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] hover:bg-indigo-50 hover:text-indigo-600 transition-all dark:text-slate-300 dark:bg-slate-800/50">
                                View Full Profile
                            </a>
                        </div>
                    </div>
