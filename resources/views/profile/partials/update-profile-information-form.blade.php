    ...
    <form method="post" action="{{ route('profile.update') }}" class="space-y-8" enctype="multipart/form-data" x-data="{ 
        ...
        <div class="flex items-center gap-8 p-6 rounded-2xl bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700">
            <div class="relative group">
                ...
                <div class="h-24 w-24 rounded-3xl p-1 bg-gradient-to-br from-indigo-500 to-purple-600 shadow-xl transition-all {{ auth()->user()->role === 'staff' ? 'cursor-default' : 'cursor-pointer group-hover:' }} overflow-hidden" @if(auth()->user()->role !== 'staff') @click="$refs.photo.click()" @endif>
                    <!-- Current Photo -->
