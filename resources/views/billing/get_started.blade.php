    ...
    <div class="flex-1 flex flex-col justify-center items-center py-12 px-4">
        ...
        <div class="mb-12 text-center">
            <div class="flex items-center justify-center gap-3 mb-6">
                <img src="{{ asset('images/logo.png') }}" alt="Velora" class="h-12 w-auto">
                <span class="text-3xl font-black text-white tracking-widest uppercase">Velora ERP</span>
            </div>
            ...
            <div class="mt-4 flex items-center justify-center gap-3 text-sm font-bold">
        ...
        <!-- Pricing Cards -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-5xl mx-auto" x-data="guestCheckout()">

            <!-- Monthly Plan -->
            <div class="bg-slate-900 border border-slate-700 rounded-[2rem] p-8 lg:p-10 relative flex flex-col justify-between hover:border-indigo-500 transition-colors">
                <div>
                    ...
                    <ul class="space-y-5 mb-10">
                        <li class="flex items-center gap-4 text-sm text-slate-300 font-bold">
                            <span class="bg-indigo-500/20 p-1 rounded"><svg class="h-5 w-5 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg></span>
                            Unlimited Clients
                        </li>
                        <li class="flex items-center gap-4 text-sm text-slate-300 font-bold">
                            <span class="bg-indigo-500/20 p-1 rounded"><svg class="h-5 w-5 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg></span>
                ...
                <button @click="pay('monthly')" :disabled="loadingPlan !== null" class="w-full bg-slate-800 hover:bg-indigo-600 text-white font-black py-4 rounded-xl border border-slate-700 hover:border-indigo-500 uppercase tracking-widest transition-all">
                    <span x-show="loadingPlan !== 'monthly'">Pay ₹999 & Continue</span>
                    <span x-show="loadingPlan === 'monthly'" class="animate-pulse">Processing...</span>
                </button>
            </div>

            <!-- Yearly Plan -->
            <div class="bg-gradient-to-b from-indigo-600 to-indigo-900 border border-indigo-500 rounded-[2rem] p-8 lg:p-10 relative flex flex-col justify-between shadow-2xl shadow-indigo-900/50 md:-translate-y-4">
                ...
                <div>
                    ...
                    <ul class="space-y-5 mb-10">
                        <li class="flex items-center gap-4 text-sm text-white font-bold">
                            <span class="bg-indigo-400/30 p-1 rounded border border-indigo-400"><svg class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg></span>
                ...
                <button @click="pay('yearly')" :disabled="loadingPlan !== null" class="w-full bg-white text-indigo-900 font-black py-4 rounded-xl shadow-lg hover:bg-slate-100 uppercase tracking-widest transition-transform hover:-translate-y-1">
                    <span x-show="loadingPlan !== 'yearly'">Pay ₹9,990 & Continue</span>
                ...
                async pay(plan) {
                    ...
                    try {
                        ...
                        const options = {
                            ...
                            "currency": "INR",
                            "name": "Velora ERP",
                            "description": plan === 'yearly' ? "Professional Yearly Plan" : "Starter Monthly Plan",
                            ...
                            "handler": async (response) => {
                                ...
                                if (verifyData.success) {
                                    // Payment done! Go to registration
