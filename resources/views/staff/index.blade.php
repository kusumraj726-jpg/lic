    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-3xl font-extrabold text-gray-900">Team Members</h2>
                    <p class="text-gray-500 mt-1">Manage your agency staff and their access status.</p>
                </div>
                <div class="flex items-center gap-4">
                    <form action="{{ route('staff.index') }}" method="GET" class="relative group">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search team members..." class="pl-10 pr-4 py-2.5 rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 bg-white shadow-sm w-64 transition-all">
                    ...
                    <a href="{{ route('staff.create') }}" class="premium-btn premium-btn-primary flex items-center gap-2 shadow-lg shadow-indigo-100">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 p-0.5 shadow-sm border border-slate-100 flex-shrink-0">
                                                <div class="h-full w-full rounded-[9px] bg-white flex items-center justify-center overflow-hidden">
                                                    @if($member->staffUser && $member->staffUser->avatar)
                                                        <img src="{{ asset('storage/' . $member->staffUser->avatar) }}" class="h-full w-full object-cover">
                                                    @else
                                            ...
                                            <div class="min-w-0">
                                                <div class="font-bold text-gray-900 truncate">{{ $member->name }}</div>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-5 text-sm font-bold uppercase tracking-wider">
                                            <a href="{{ route('staff.show', $member) }}" class="text-indigo-600 hover:text-indigo-900 flex items-center gap-1.5 group transition-transform hover:scale-105">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                View
                                            </a>
                                            <a href="{{ route('staff.edit', $member) }}" class="text-amber-600 hover:text-amber-900 flex items-center gap-1.5 group transition-transform hover:scale-105">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                                Edit
                                            </a>
                                            <form action="{{ route('staff.destroy', $member) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="text-rose-600 hover:text-rose-900 flex items-center gap-1.5 group transition-transform hover:scale-105" onclick="return confirm('Move staff member to trash?')">
