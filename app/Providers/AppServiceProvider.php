...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        view()->composer('layouts.app', function ($view) {
            ...
            // Intelligence Logic
            if ($open_queries > 5) {
                $briefs->push(['title' => 'Query Surge', 'message' => 'High communication volume detected.', 'type' => 'warning', 'url' => route('queries.index')]);
            }
            if ($pending_claims > 3) {
                $briefs->push(['title' => 'Claim Pulse', 'message' => 'Multiple cases pending resolution.', 'type' => 'info', 'url' => route('claims.index')]);
            }
            if ($upcoming_renewals_7d > 0) {
                $briefs->push(['title' => 'Renewal Alert', 'message' => "{$upcoming_renewals_7d} renewals in 7 days.", 'type' => 'success', 'url' => route('renewals.index')]);
            }
